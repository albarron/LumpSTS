...
java -cp target/LumpSTS-0.0.1-SNAPSHOT-jar-with-dependencies.jar cat.lump.sts2017.similarity.SentenceW2VSimCalculator -e $embeddingsLAN  -s $path$source -t $path$target -m cosine -o $path${simPath}BN$elang.cosine
java -cp target/LumpSTS-0.0.1-SNAPSHOT-jar-with-dependencies.jar cat.lump.sts2017.similarity.SentenceW2VSimCalculator -e $embeddingsLAN  -s $path$source -t $path$target -m jaccard -o $path${simPath}BN$elang.jaccard
#java -cp target/LumpSTS-0.0.1-SNAPSHOT-jar-with-dependencies.jar cat.lump.sts2017.similarity.SentenceW2VSimCalculator -e $embeddingsLAN  -s $path$source -t $path$target -m KL -o $path${simPath}BN$elang.kl
#java -cp target/LumpSTS-0.0.1-SNAPSHOT-jar-with-dependencies.jar cat.lump.sts2017.similarity.SentenceW2VSimCalculator -e $embeddingsLAN  -s $path$source -t $path$target -m JS -o $path${simPath}BN$elang.js
